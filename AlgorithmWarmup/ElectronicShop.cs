...
namespace ProblemSolving
{
    public class ElectronicShop
    {
        ...
        {
            int maxDevicesBought = -1;

            for (var i = 0; i < keyboards.Length; i++)
            {
                for (var j = 0; j < drives.Length; j++)
                {
                    if (keyboards[i] + drives[j] > maxDevicesBought && keyboards[i] + drives[j] <= b)
                    {
                        maxDevicesBought = keyboards[i] + drives[j];
                    }
                }
            }
            return maxDevicesBought;
        }
        ...
        The first line contains three space-separated integers , , and , the budget, the
        number of keyboard models and the number of USB drive models.
