...
namespace ProblemSolving
{
    public class Funny
    {
        public static string FunnyString(string s)
        {
            ...
            // Get the ASCII values for the original String
            byte[] sASCII = Encoding.ASCII.GetBytes(s);

            // Get the ASCII values for the reversed (Copy) String
            byte[] reversedStrASCII = Encoding.ASCII.GetBytes(reversedStr);

            // Declare empty Lists to hold the new values to compare
            var sAbs = new List<int>();
            var reversedStrAbs = new List<int>();

            for (var i = 0; i < s.Length - 1; i++)
            {
                var diffS = Math.Abs(sASCII[i] - sASCII[i + 1]);
                //Array.Fill<int>(sAbs, diffS);
                sAbs.Add(diffS);
            }

            for (var i = 0; i < s.Length - 1; i++)
            {
                var diffR = Math.Abs(reversedStrASCII[i] - reversedStrASCII[i + 1]);
                //Array.Fill<int>(reversedStrAbs, diffR);
                reversedStrAbs.Add(diffR);
            }

            if (sAbs.SequenceEqual(reversedStrAbs))
                return "Funny";
            return "Not Funny";
        }
    }
    ...
    //characters at positions 0 and 1, 1 and 2 and so on to the end. If the list of absolute
    ...
    //The ordinal values of the charcters are .  and the ordinals are.
