...
namespace Codewars
{
    public class Printer
    {
        public static string PrinterError(String s)
        {
            ...
            int errorCount = 0;
            string errorString = "nopqrstuvwxyz";
            char[] sConvertedToCharArray = s.ToCharArray();
            for (int i = 0; i < sConvertedToCharArray.Length; i++)
            {

                if (errorString.Contains(sConvertedToCharArray[i]))
                    errorCount++;
            }
            ...
            //var count = chars.Count(x => errorString.Contains(x));
...
//boxes the printer has to use colors which,
